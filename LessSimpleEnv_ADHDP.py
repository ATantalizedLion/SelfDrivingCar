...
class TrainMemory:
    ...
    def store(self, avg_reward, runtime):
        self.avg_rewards.append(avg_reward)
        self.run_times.append(runtime)

    def save(self, location):
        save_arr = np.array([self.avg_rewards, self.run_times])
        np.savetxt(location + '/Progress.csv', save_arr)
...
class CriticModel(tf.keras.Model):
    def __init__(self, learning_rate, observation_shape):
        ...
        k_initializer = tf.keras.initializers.glorot_uniform()
        ...
        self.opt = tf.keras.optimizers.Nadam(learning_rate)
...
class ActorModel(tf.keras.Model):
    def __init__(self, learning_rate, observation_shape):
        ...
        k_initializer = tf.keras.initializers.TruncatedNormal(mean=0.0, stddev=0.05)
        b_initializer = tf.keras.initializers.zeros()
        ...
        self.opt = tf.keras.optimizers.Nadam(learning_rate / 2)
...
env = gym.make('carai-less-simple-v0')  # First open the environment
...
if load_model == "Y" or load_model == "y":
    ...
    if continue_training == "Y" or continue_training == "y":
        learning_rate = 0.000001
        Actor.opt = tf.keras.optimizers.Nadam(learning_rate)
        Critic.opt = tf.keras.optimizers.Nadam(learning_rate)
        maxEpochTime = 240  # [s] max seconds to spend per epoch
        maxSteps = int(maxEpochTime / dt)  # max duration of an epoch
        Train = True
    else:
        Train = False
...
while epoch < maxEpoch:
    ...
    while i < maxSteps:
        ...
        if done:
            break
        i += 1
    # if Train and sum(mem.rewards) / len(mem.rewards) > -0.1:  # -0.0999 highest observed award,
    #     # when reached, quit training due to current instability of the method.
    #     Train = False
    #     epoch = maxEpoch * 2
    if Train:
        ...
        # Statistics for (live and hindsight) training progress evaluation
        reward_average = sum(mem.rewards) / len(mem.rewards)
        train_mem.store(reward_average, info_dict['t'])
...
if save == "Y" or save == "y":
    ...
    save_location = 'Models/' + model_save_name
    actor_save_name = save_location + '/Actor'
    critic_save_name = save_location + '/Critic'

    tf.keras.models.save_model(Actor, actor_save_name)
    tf.keras.models.save_model(Critic, critic_save_name)

    train_mem.save(save_location)
