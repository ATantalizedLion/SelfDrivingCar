...
class CriticModel(tf.keras.Model):
    def __init__(self, learning_rate, observation_shape):
        ...
        self.value = tf.keras.layers.Dense(observation_shape)
...
class ActorModel(tf.keras.Model):
    def __init__(self, learning_rate, observation_shape):
        ...
        self.dense1 = tf.keras.layers.Dense(neurons_inner_layer, activation='relu',
                                            kernel_initializer=k_initializer, bias_initializer=b_initializer)
        self.dense2 = tf.keras.layers.Dense(neurons_inner_layer, activation='relu',
                                            kernel_initializer=k_initializer, bias_initializer=b_initializer)
        self.control = tf.keras.layers.Dense(2, activation=map_to_range)
...
def get_loss_critic(Critic, memory, gamma=0.99):
    ...
    # print(tf.convert_to_tensor(np.array(discounted_rewards)[:, None], dtype=tf.float32)) - size = 130 1 10
    # print(values) - size = 130 2 10
    reward_tensor = tf.convert_to_tensor(np.array(discounted_rewards)[:, None], dtype=tf.float32)
    reward_tensor2 = tf.concat([reward_tensor, reward_tensor], 1)  # same for both
    advantage = - values
...
while epoch < maxEpoch:
    ...
    while i < maxSteps:
        ...
        obs, reward, done, info_dict, Terminate = env.step(action, dt)
        mem.store(obs, action, reward, info_dict['JStar'])
